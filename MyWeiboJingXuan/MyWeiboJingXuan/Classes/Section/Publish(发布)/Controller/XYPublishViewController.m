...
@interface XYPublishViewController ()
@end

@implementation XYPublishViewController
...
- (void)viewDidLoad {
    ...
    CGFloat buttonStartY = (XYSCREEN_H - 2 * buttonH) * 0.5;
    CGFloat buttonStartX = 20;
    CGFloat xMargin = (XYSCREEN_W - 2 * buttonStartX - maxCols * buttonW) / (maxCols - 1);
    for (int i = 0; i<images.count; i++) {
        XYVerticalButton *button = [[XYVerticalButton alloc] init];
        button.tag = i;
        [button addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:button];
        // 设置内容
        button.titleLabel.font = XYTagFont;
    ...
    CGFloat centerX = XYSCREEN_W * 0.5;
    CGFloat centerEndY = XYSCREEN_H * 0.2;
    CGFloat centerBeginY = centerEndY - XYSCREEN_H;
    sloganView.xy_centerY = centerBeginY;
    sloganView.xy_centerX = centerX;
    anim.fromValue = [NSValue valueWithCGPoint:CGPointMake(centerX, centerBeginY)];
    anim.toValue = [NSValue valueWithCGPoint:CGPointMake(centerX, centerEndY)];
    anim.beginTime = CACurrentMediaTime() + images.count * XYAnimationDelay;
    anim.springBounciness = XYSpringFactor;
    anim.springSpeed = XYSpringFactor;
